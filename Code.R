powerConsum <- read.csv("household_power_consumption.txt", sep = ";", stringsAsFactors = FALSE) #Reading CSV file
powerConsum$Date <- as.Date(powerConsum$Date, format = "%d/%m/%Y") #Making change in classes of Date and Time variables
powerConsum$DateTime <- strptime(paste(powerConsum$Date,powerConsum$Time),
                                 format = "%Y-%m-%d %H:%M:%S")
powerConsum <- powerConsum[powerConsum$Date >= "2007-2-1" & powerConsum$Date <= "2007-2-2",] #Selecting Subset Data from "2007-2-1" to "2007-2-2"
powerConsum[,3] <- as.numeric(powerConsum[,3]) #Making change in classes of Data's 3:8 columns
